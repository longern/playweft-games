...
function on_action(state, action, context)
  ...
  if action.type == "draw" then
    local card = draw_one(state, context.playerId)
    ...
    return { state = state, events = { { type = "drew", player = context.playerId } } }
  end
  ...
  if draw_count > 0 then
    ...
    return { state = state, events = { { type = "penalty", player = context.playerId, count = drawn } } }
  end
  ...
  if card.value == "reverse" then
    ...
    return { state = state, events = { { type = "reversed", player = context.playerId } } }
  end
  ...
  return { state = state, events = { { type = "played", player = context.playerId, card = card.value } } }
end
