...
function on_action(state, action, context)
  ...
  if state.phase == "bidding" then
    ...
    state.bids[context.playerId] = score
    ...
    if score == 3 then
      start_playing(state)
      return { state = state, events = { { type = "landlord", player = context.playerId } } }
    end
    if state.bidCount == #state.players then
      if state.highestBid == 0 then
        redeal(state)
        return { state = state, events = { { type = "redealt" } } }
      end
      start_playing(state)
      return { state = state, events = { { type = "landlord", player = state.landlord } } }
    end
    advance_turn(state)
    state.lastEvent = { kind = score == 0 and "pass_bid" or "bid", playerIndex = index, count = score }
    return { state = state, events = { { type = "bid", player = context.playerId, score = score } } }
  end

  if state.phase ~= "playing" then return rejected(state, "invalid_phase") end
  if action.type == "pass" then
    if not state.lastPlay or state.lastPlay.playerId == context.playerId then
    ...
    if state.passCount >= #state.players - 1 then
      ...
      return { state = state, events = { { type = "new_trick", player = state.players[leader] } } }
    end
    state.lastEvent = { kind = "pass", playerIndex = index, count = 0 }
    advance_turn(state)
    return { state = state, events = { { type = "pass", player = context.playerId } } }
  end

  if action.type ~= "play" then return rejected(state, "invalid_action") end
  local cards = selected_cards(state, context.playerId, action.cards)
  ...
  state.hands[context.playerId] = remove_cards(state.hands[context.playerId], cards)
  state.lastPlay = {
  ...
  if #state.hands[context.playerId] == 0 then
    state.winner = context.playerId
    state.winnerIndex = index
    state.winnerTeam = context.playerId == state.landlord and "landlord" or "farmers"
  ...
  advance_turn(state)
  return { state = state, events = { { type = "played", player = context.playerId, cards = #cards } } }
end
...
function on_player_left(state, context)
  ...
  if state.landlord ~= "" and context.playerId ~= state.landlord then
