...
export function normalizedConfig(config) {
  const roles = config.roles
    .map((role, index) => ({
      id: String(role.id || `custom_${index + 1}`).slice(0, 40),
      name: String(role.name || "自定义神职").trim().slice(0, 24),
      mark: String(role.mark || role.name || "?").trim().slice(0, 1).toUpperCase() || "?",
      team: ["wolf", "villager", "god", "other"].includes(role.team) ? role.team : "god",
      copy: String(role.copy || "自定义神职，技能按版型规则执行。").trim().slice(0, 160),
      count: Math.max(0, Math.min(12, Math.round(Number(role.count) || 0))),
    }))
    .filter((role) => role.count > 0 && role.name);
  return { presetId: String(config.presetId || "custom"), name: String(config.name || "自定义版型").trim().slice(0, 40), rules: String(config.rules || "").trim().slice(0, 1000), roles };
}
