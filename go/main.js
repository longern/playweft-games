...
const client = createPlayweftClient({
  ...
  onError(error, _code, requestId) {
    ...
    elements.message.textContent = translateError(error);
...
elements.handicapSetting.addEventListener("change", updateHandicapControls);
...
function renderSetup(nextState, ownIndex) {
  ...
  const canConfigure =
    Boolean(state) && ownPlayer === hostId && !pendingActionId;
  const settings = nextState.settings ?? preview.settings;
  elements.kicker.textContent = nextState.round > 1 ? "调整下一局" : "开局准备";
  elements.heading.textContent = "设置本局棋盘";
  ...
  for (const control of elements.setupForm.elements) {
