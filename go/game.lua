...
function setup(context)
  ...
  return setup_state(
    ...
    1,
    settings,
    math.floor(math.abs(context.randomSeed or 1)),
    context.hostId or context.ownerId or context.players[1]
  )
end
...
function on_action(state, action, context)
  ...
  if state.phase == "setup" then
    ...
    if context.playerId ~= state.hostId then return rejected(state, "only_host_can_setup") end
    ...
    return {
      state = next_state,
      events = { { type = "started", player = context.playerId } },
    }
  end
  ...
  if action.type == "rematch" then
    ...
    return {
      state = next_state,
      events = { { type = "rematched", player = context.playerId } },
    }
  end
  ...
  if action.type == "configure" then
    ...
    return {
      state = next_state,
      events = { { type = "configuration_opened", player = context.playerId } },
    }
  end
  ...
  if action.type == "pass" then
    ...
    return {
      state = state,
      events = { { type = "passed", player = context.playerId } },
    }
  end
...
function on_player_left(state, context)
  ...
  return {
    state = state,
    events = { { type = "player_left", player = context.playerId } },
  }
end
